...
This record identifies the played campaign `Panství Penbrok`. `Dopisy z panství Penbrok` is the title of Yann du Bois's player-authored source and account, not the campaign title. The letters are a character-perspective campaign account, not an objective reconstruction.
...
All ten dated sections name `Příběhy impéria` as the RPG system. Derived play metadata uses the curator-confirmed date 2023-05-17 for `Prokletý klášter` and 2024-04-21 for `Hořkosladké vítězství`, while retaining the respective source-stated values `22.2.2023` and `21.4.20234`. These are real-world session dates only and do not date fictional events.
